...
const nav = [
  { group:'DASHBOARD', items:[
    ...
    { id:'market', label:'Data Pulse', icon:Globe2 },
    { id:'news', label:'Source Feed', icon:Newspaper },
  ]},
  { group:'GENLAYER INTELLIGENCE', items:[
    { id:'research', label:'Intelligence Desk', icon:Bot },
    { id:'reports', label:'AI Provenance', icon:Sparkles },
    { id:'signals', label:'Signal Attestation', icon:Gauge },
  ]},
  { group:'POLICY STUDIO', items:[
    { id:'spot', label:'Asset Intent', icon:CircleDollarSign },
    { id:'futures', label:'Derivative Intent', icon:Zap },
    { id:'orders', label:'Intent Ledger', icon:Briefcase },
    { id:'automation', label:'Automation', icon:CalendarClock },
    { id:'localtrade', label:'Local Runtime', icon:Play },
    { id:'jinbot', label:'Runtime Signals', icon:Bot },
  ]},
  { group:'SYSTEM', items:[
    { id:'api', label:'Contract/API Health', icon:Database },
...
function liveLabel(){ return 'Live adapter context for GenLayer policy decisions'; }
...
export default function Page(){
  ...
  const [aiQ,setAiQ] = useState('Analyze this BTC policy intent using live adapter depth, source feed context and GenLayer-ready risk rules.');
  ...
  async function loadJinbot(){
    try{
      ...
      setJinbot(j);
    }catch(e:any){ setJinbot({ok:false,error:e?.message||'Local runtime bridge unavailable'}); }
  }
  async function sendJinbotCommand(cmd?:string){
    ...
    try{
      ...
      setJinbotResult(j);
      await loadJinbot();
    }catch(e:any){ setJinbotResult({ok:false,error:e?.message||'Local runtime command failed'}); }
  ...
  function scheduleSpotAutoClose(openOrder:any, source:'manual'|'automation'='manual'){
    ...
    window.setTimeout(async()=>{
      ...
      setAutoLog(x=>[{time:new Date().toLocaleTimeString(), mode:liveIntent?'live asset close':'preview asset close', result:j},...x].slice(0,10));
    }, delayMs);
  }
  ...
  function scheduleFuturesAutoClose(openOrder:any, source:'manual'|'automation'='manual'){
    ...
    window.setTimeout(async()=>{
      ...
      try{
        ...
        setAutoLog(x=>[{time:new Date().toLocaleTimeString(), mode:liveIntent?'live derivative auto-close':'preview derivative auto-close', result:j},...x].slice(0,10));
      }catch(e:any){
  ...
  async function execute(){
    ...
    if(liveIntent && !confirm('Execution adapter is armed. Submit this intent now?')) return;
    setExecuting(true);
    try{
  ...
  return <main className="min-h-screen bg-[#05070d] grid-bg text-slate-100 overflow-x-hidden">
    <aside className="fixed inset-y-0 left-0 w-[250px] bg-[#070b15]/95 border-r border-slate-800 p-4 overflow-y-auto z-30">
      ...
      <div className="glass rounded-2xl p-4 mt-4"><b className="flex gap-2 items-center"><Wallet size={17}/>{wallet?'Wallet Connected':'No Wallet Connected'}</b><p className="text-xs text-slate-400 mt-2 break-all">{wallet || 'Connect wallet to unlock account-linked policy panels.'}</p></div>
    ...
    <section className="pl-[250px] min-h-screen">
      <header className="sticky top-0 z-20 bg-[#050b16]/92 backdrop-blur-xl border-b border-slate-800">
        <div className="h-[72px] px-5 flex items-center gap-4">
          ...
          <div className="hidden xl:flex items-center gap-2 text-sm"><span>Built for</span><b className="text-violet-300">GenLayer Studio</b><span>×</span><b className="text-cyan-300">Policy Intent Runtime</b></div>
...
function TradingLayout(props:any){ const {active,terminal,coins,news,signals,symbol,setSymbol,interval,setIntervalState,order,setOrder,paperMode,setPaperMode,serverLiveTrading,preview,execute,executing,execResult,pendingSpotCloses,pendingFuturesCloses,adminSecret,setAdminSecret,wallet}=props; const base=baseSymbol(symbol); const mid=Number(terminal?.ticker?.price||0); return <div className="grid grid-cols-12 gap-4">
  ...
  <Card className="col-span-12 xl:col-span-4"><h3 className="text-xl font-black mb-3">Policy Positions</h3><PolicyPositions data={terminal?.account?.positions}/></Card>
  <Card className="col-span-12 xl:col-span-3"><h3 className="text-xl font-black mb-3">Policy Signals</h3>{signals.slice(0,5).map((s:any)=><SignalRow key={s.id} s={s}/>)}</Card>
  <Card className="col-span-12 xl:col-span-2"><h3 className="text-xl font-black mb-3">Live Adapter Events</h3><Trades data={(terminal?.trades||[]).slice(0,12)}/></Card>
  ...
  <Card className="col-span-12"><h3 className="text-xl font-black mb-3">Source Feed & Intelligence</h3><div className="grid md:grid-cols-4 gap-4">{news.length?news.slice(0,4).map((n:any,i:number)=><div key={i} className="pill rounded-2xl p-4"><span className="text-xs text-cyan-300">RESEARCH</span><h4 className="font-black mt-2">{n.title||n.newsTitle||n.content||'Live research item'}</h4><p className="text-xs text-slate-400 mt-2">source-backed live feed</p></div>):<Empty title="No live source items" text="The source adapter returned no items."/>}</div></Card>
 </div> }
...
function AccountSummary({account}:any){ const balances=account?.balances||[]; return <div><MetricTiny label="Connection" value={account?.connected?'Adapter connected':'Unavailable'}/><MetricTiny label="Balances" value={Array.isArray(balances)?balances.length:'—'}/><MetricTiny label="Policy Positions" value={Array.isArray(account?.positions)?account.positions.length:'—'}/><MetricTiny label="Open Intents" value={Array.isArray(account?.orders)?account.orders.length:'—'}/></div> }
...
function NewsFeed({news}:any){ return <Card><h3 className="text-xl font-black mb-4">Source Feed</h3>{news.length?news.map((n:any,i:number)=><div key={i} className="py-4 border-b border-slate-800"><span className="text-xs text-violet-300">LIVE RESEARCH</span><h4 className="font-black text-lg">{n.title||n.newsTitle||n.content||'Live research item'}</h4><p className="text-sm text-slate-400 mt-1">Source Adapter</p></div>):<Empty title="No live source items" text="Source Adapter returned no current feed."/>}</Card> }
function MarketIntelligence({live,terminal,news,signals,coins}:any){
  ...
  return <div className="grid grid-cols-12 gap-4">
    <Card className="col-span-12"><h3 className="text-2xl font-black mb-2">Intelligence Desk</h3><p className="text-slate-400">Live market overview from execution-adapter depth and source-backed research. This page is for reading policy context, not generating AI reports.</p></Card>
...
function OrdersPositions({terminal,execResult}:any){ return <div className="grid grid-cols-12 gap-4"><Card className="col-span-12 xl:col-span-6"><h3 className="text-xl font-black mb-3">Policy Positions</h3><PolicyPositions data={terminal?.account?.positions}/></Card><Card className="col-span-12 xl:col-span-6"><h3 className="text-xl font-black mb-3">Open Intents</h3><PolicyPositions data={terminal?.account?.orders}/></Card><Card className="col-span-12"><h3 className="text-xl font-black mb-3">Latest Policy / Execution Response</h3><pre className="no-data rounded-2xl p-4 whitespace-pre-wrap">{JSON.stringify(execResult||{status:'No policy preview yet'},null,2)}</pre></Card></div> }
function Portfolio({wallet,terminal}:any){ if(!wallet) return <Empty title="Connect wallet" text="No portfolio or account details are shown until a real wallet is connected."/>; return <div className="grid grid-cols-12 gap-4"><Card className="col-span-12"><h3 className="text-xl font-black">Connected Wallet</h3><p className="break-all text-cyan-200 mt-2">{wallet}</p></Card><Card className="col-span-12"><h3 className="text-xl font-black mb-3">Execution Adapter Account</h3><pre className="no-data rounded-2xl p-4 max-h-[500px] overflow-auto">{JSON.stringify(terminal?.account||{},null,2)}</pre></Card></div> }
function Automation({order,setOrder,autoOn,setAutoOn,runAutomation,autoLog,paperMode,serverLiveTrading}:any){ return <Card><h3 className="text-2xl font-black flex gap-2"><CalendarClock/>Automation Guard</h3><p className="text-slate-400 mt-2">Run scheduled policy previews from the browser; execution requires Vercel ENV and adapter-armed mode.</p><div className="grid md:grid-cols-6 gap-3 mt-5"><select value={order.schedule} onChange={e=>setOrder({...order,schedule:e.target.value})} className="input"><option value="off">Manual only</option><option value="preview">Auto Preview</option><option value="live">Auto Execute if armed</option></select><select value={order.frequency} onChange={e=>setOrder({...order,frequency:e.target.value})} className="input"><option>1m</option><option>5m</option><option>15m</option><option>1h</option><option>4h</option></select><input value={order.maxNotional} onChange={e=>setOrder({...order,maxNotional:e.target.value})} className="input" placeholder="Max notional"/><select value={order.spotAutoClose} onChange={e=>setOrder({...order,spotAutoClose:e.target.value})} className="input"><option value="off">No asset close</option><option value="random">Asset close 1-3m</option></select><select value={order.futuresAutoClose} onChange={e=>setOrder({...order,futuresAutoClose:e.target.value})} className="input"><option value="off">No derivative close</option><option value="time">Derivative time close</option></select><button onClick={()=>setAutoOn(!autoOn)} className={`rounded-2xl px-4 font-bold ${autoOn?'bg-rose-600':'bg-emerald-600'}`}>{autoOn?'Stop Auto':'Start Auto'}</button><button onClick={runAutomation} className="rounded-2xl bg-slate-800 px-4 font-bold">Run Now</button><div className="pill rounded-2xl p-3 text-sm">{paperMode?'Preview':'Execute'} · {serverLiveTrading?'Adapter armed':'Adapter locked'}</div></div><div className="mt-5 grid md:grid-cols-2 gap-3">{autoLog.map((x:any,i:number)=><pre key={i} className="no-data rounded-2xl p-3 text-[11px] max-h-52 overflow-auto">{x.time} · {x.mode}\n{JSON.stringify(x.result,null,2).slice(0,900)}</pre>)}</div></Card> }
function ApiHealth({health}:any){ return <Card><h3 className="text-xl font-black mb-3">Contract/API Health</h3><pre className="no-data rounded-2xl p-5 whitespace-pre-wrap">{JSON.stringify(health||{status:'loading'},null,2)}</pre></Card> }
  ...
  return <div className="grid grid-cols-12 gap-4">
    <Card className="col-span-12">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h3 className="text-2xl font-black flex items-center gap-2"><Play/>Local Runtime</h3>
          <p className="text-slate-400 mt-2">Realtime execution is handled by the local automation runtime; this panel controls its dashboard bridge while GenLayer Studio stays deployed.</p>
        </div>
      ...
      <div className="grid md:grid-cols-4 gap-3 mt-5">
        <div className="pill rounded-2xl p-4"><p className="text-xs text-slate-400">RUNTIME</p><b className={bridgeReady?'text-emerald-300':'text-amber-300'}>{bridgeReady?'Local online':'Start local runtime'}</b></div>
    ...
    <Card className="col-span-12 xl:col-span-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <button onClick={()=>run('/start_perps')} disabled={busy} className="rounded-xl bg-emerald-600 py-3 font-black">Start Derivative</button>
        <button onClick={()=>run('/stop_perps')} disabled={busy} className="rounded-xl bg-slate-800 py-3 font-black">Stop Derivative</button>
        <button onClick={()=>run('/start_spot')} disabled={busy} className="rounded-xl bg-cyan-700 py-3 font-black">Start Asset</button>
        <button onClick={()=>run('/stop_spot')} disabled={busy} className="rounded-xl bg-slate-800 py-3 font-black">Stop Asset</button>
      ...
      <button onClick={()=>run('/close_position')} disabled={busy} className="mt-3 w-full rounded-2xl bg-amber-600 py-3 font-black">Close Derivative Position</button>
    ...
    <Card className="col-span-12 xl:col-span-4">
      <h3 className="text-xl font-black mb-3">Runtime Status</h3>
      ...
      <MetricTiny label="Derivative pair" value={data?.tradeSettings?.futuresSymbol || perps?.symbol || 'Unavailable'}/>
      <MetricTiny label="Asset pair" value={data?.tradeSettings?.spotSymbol || spot?.symbol || 'Unavailable'}/>
    ...
    <Card className="col-span-12 xl:col-span-6">
      <h3 className="text-xl font-black mb-3">Local Execution Events</h3>
      {trades.length?trades.map((t:any,i:number)=><div key={i} className="py-2 border-b border-slate-800 text-sm"><b>{t.symbol||t.market||t.side||'Trade'}</b><p className="text-slate-400">{JSON.stringify(t).slice(0,260)}</p></div>):<Empty title="No local trades" text="Local execution log has not returned entries."/>}
    </Card>
  </div>
}
...
function JinbotCrossPanel({data,refresh,command,setCommand,secret,setSecret,send,busy,result}:any){
  ...
  const statusText = data?.ok ? 'Connected' : 'Waiting for local runtime';
  return <div className="grid grid-cols-12 gap-4">
    <Card className="col-span-12 xl:col-span-8">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="text-2xl font-black flex items-center gap-2"><Bot/>Local Runtime Signals</h3>
          <p className="text-slate-400 mt-2">Dedicated signal menu connected to the local automation runtime through bridge or Telegram commands.</p>
        </div>
        <button onClick={refresh} className="pill rounded-2xl px-4 py-3 flex items-center gap-2"><RefreshCw size={16}/>Refresh</button>
      </div>
      <div className="grid md:grid-cols-4 gap-3 mt-5">
        <div className="pill rounded-2xl p-4"><p className="text-xs text-slate-400">RUNTIME STATUS</p><b className={data?.ok?'text-emerald-300':'text-amber-300'}>{statusText}</b></div>
        ...
        <div className="pill rounded-2xl p-4"><p className="text-xs text-slate-400">COMMAND CHANNEL</p><b className={data?.configured?.telegram?'text-emerald-300':'text-amber-300'}>{data?.configured?.telegram?'Ready':'Not configured'}</b></div>
      </div>
      <div className="mt-5 grid lg:grid-cols-2 gap-3">
        {signals.length ? signals.slice(0,10).map((s:any)=><div key={s.id} className="holo-card rounded-2xl p-4">
          <div className="flex justify-between gap-3"><div><b className="text-lg">{s.symbol}</b><p className="text-xs text-slate-400">{s.regime||'LOCAL POLICY SIGNAL'}</p></div><span className={String(s.side).includes('LONG')||String(s.side).includes('BUY')?'text-emerald-300 font-black':'text-rose-300 font-black'}>{s.side}</span></div>
        ...
        </div>) : <Empty title="No local signal yet" text="Start the local runtime, expose its dashboard bridge, or configure Telegram variables for command forwarding."/>}
      </div>
    </Card>
    <Card className="col-span-12 xl:col-span-4">
      <h3 className="text-xl font-black mb-2">Runtime Command Channel</h3>
      <p className="text-sm text-slate-400 mb-4">Send runtime commands without leaving GenLayer Intelligent Finance Studio. If JINBOT_PANEL_SECRET or ADMIN_SECRET is set, enter it below.</p>
      <input value={command} onChange={e=>setCommand(e.target.value)} className="input" placeholder="/status or /signal"/>
      <input type="password" value={secret} onChange={e=>setSecret(e.target.value)} className="input" placeholder="JINBOT_PANEL_SECRET / ADMIN_SECRET" autoComplete="off"/>
      <button onClick={()=>send()} disabled={busy} className="w-full rounded-2xl bg-gradient-to-r from-violet-600 to-cyan-500 py-3 font-black">{busy?'Sending...':'Send to Runtime'}</button>
      ...
      <div className="mt-5 no-data rounded-2xl p-3 text-xs"><b>Bridge endpoints</b><p className="mt-2">Status: {data?.endpoints?.status||'unavailable'}</p><p>Signals: {data?.endpoints?.signals||'unavailable'}</p><p>Policy Positions: {data?.endpoints?.positions||'unavailable'}</p></div>
      ...
      {!data?.ok&&<pre className="mt-4 no-data rounded-2xl p-3 text-[11px] max-h-80 overflow-auto whitespace-pre-wrap">{JSON.stringify(data?.diagnostics||data||{status:'Local runtime not loaded yet'},null,2)}</pre>}
    </Card>
  </div>
}
